...
<body>
  ...
  <?php }} ?>
                ...
                <ul class="navbar-nav align-self-stretch">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active" href="dailyreports.php"> <i class="fa fa-trophy  mr-3 text-primary"></i>
                            Daily Reports</a>
                    </li>
                    <li class="nav-item">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-striped bg-light small">
                                            ...
                                            <?php
                                                ...
                                                $sql = "SELECT * FROM task WHERE email='{$_SESSION['legitEmail']}' limit $page1,5";
                                                $data = mysqli_query($conn,$sql);
                                               // $result = $conn->query($sql) or die($conn->error);

                                              //  $row = mysqli_fetch_assoc($);
                                                    while($row = $data->fetch_assoc()) {
                                                        echo '<tr><td class="text-uppercase">' . $row["taskname"]. '</td><td>' . $row["assdate"]. '</td><td >' . $row["deadline"]. '</td><td >' . $row["status"]. '</td></tr>';
                                                    }
                                        ...
                                        <nav aria-label="Page navigation example">
                                            <ul class="pagination justify-content-center">
                                                <?php 
                                                $sql = "SELECT * FROM task WHERE email='{$_SESSION['legitEmail']}'";
                                                $data = mysqli_query($conn,$sql);
                                                if($data){
